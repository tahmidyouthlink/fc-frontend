...
const EditProductPage = ({ searchParams }) => {
  ...
  const season = searchParams.season; // Get the 'season' query parameter
    ...
    try {
      ...
      if (res.data.modifiedCount > 0) {
        ...
        if (decodedSeasonName) {
          router.push(`/dash-board/product-hub/products/existing-products/seasons/${decodedSeasonName}`);
          return;
        }
        else {
          router.push(`/dash-board/product-hub/products/existing-products/${selectedCategory}`);
        }
      } else {
