...
const EditTransferOrder = () => {
      ...
      if (result.isConfirmed) {
        try {
          ...
          if (res?.data?.deletedCount) {
            ...
            router.push("/dash-board/product-hub/transfers")
          }
        } catch (error) {
  ...
  const handleReceiveTransferClick = () => {
    router.push(`/dash-board/product-hub/transfers/receive-transfer/${id}`);
  }
    ...
    <div className='bg-gray-50 min-h-[calc(100vh-60px)] px-6'>
      ...
      ${transferOrderStatus === "pending" ? "bg-yellow-100 text-yellow-600"
          ...
          <div className='flex justify-between md:justify-end gap-4 items-center w-full'>
            ...
            <Link className='flex items-center gap-2 text-[10px] md:text-base justify-end' href={"/dash-board/product-hub/transfers"}> <span className='border border-black hover:scale-105 duration-300 rounded-full p-1 md:p-2'><FaArrowLeft /></span> Go Back</Link>
          </div>
        </div>
      </div>
