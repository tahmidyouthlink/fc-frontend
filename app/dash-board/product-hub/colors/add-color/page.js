  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (response.status === 201) {
        ...
        reset();
        router.push("/dash-board/product-hub/colors")
      }
    } catch (error) {
    ...
    <div className='relative px-6 bg-gray-50 min-h-screen'>
      ...
      <div className='max-w-screen-xl mx-auto pt-3 md:pt-6'>
        <div className='flex items-center justify-between'>
          ...
          <Link className='flex items-center gap-2 text-[10px] md:text-base justify-end w-full' href={"/dash-board/product-hub/colors"}> <span className='border border-black hover:scale-105 duration-300 rounded-full p-1 md:p-2'><FaArrowLeft /></span> Go Back</Link>
        </div>
      </div>
