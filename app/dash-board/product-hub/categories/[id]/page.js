  ...
  const onSubmit = async (data) => {
    try {
      ...
      if (res.data.modifiedCount > 0) {
        ...
        router.push('/dash-board/product-hub/categories');
      } else {
    ...
    <form onSubmit={handleSubmit(onSubmit)} className='bg-gray-50'>
      ...
      <div className='max-w-screen-lg mx-auto pt-3 md:pt-6 px-6'>
        <div className='flex items-center justify-between'>
          ...
          <Link className='flex items-center gap-2 text-[10px] md:text-base justify-end w-full' href={"/dash-board/product-hub/categories"}> <span className='border border-black hover:scale-105 duration-300 rounded-full p-1 md:p-2'><FaArrowLeft /></span> Go Back</Link>
        </div>
      </div>
