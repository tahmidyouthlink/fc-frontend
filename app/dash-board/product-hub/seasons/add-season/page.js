  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (response.status === 201) {
        ...
        router.push("/dash-board/product-hub/seasons")
      } else {
    ...
    <div className='bg-gray-50 min-h-screen'>
      ...
      <div className='max-w-screen-lg mx-auto pt-3 md:pt-6 px-6'>
        <div className='flex items-center justify-between'>
          ...
          <Link className='flex items-center gap-2 text-[10px] md:text-base justify-end w-full' href={"/dash-board/product-hub/seasons"}> <span className='border border-black hover:scale-105 duration-300 rounded-full p-1 md:p-2'><FaArrowLeft /></span> Go Back</Link>
        </div>
      </div>
