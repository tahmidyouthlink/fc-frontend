...
const EditRefundPolicyPage = () => {
  ...
  useEffect(() => {
    const fetchRefundPolicyDetails = async () => {
      try {
        const { data } = await axiosPublic.get(`/get-single-refund-policy/${id}`);
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const refundPolicyData = {
        ...
        refundAndPolicy: data?.refundPolicy,
      }

      const res = await axiosPublic.put(`/update-refund-policies/${id}`, refundPolicyData);
      ...
      if (res.data.modifiedCount > 0) {
              ...
              <div className="flex items-start">
                <div className="ml-3 flex-1">
                  ...
                  <p className="mt-1 text-sm text-gray-500">
                    Refund Policy has been updated successfully!
                  </p>
                </div>
              </div>
            </div>
        ...
        router.push("/dash-board/settings/refund-policy");
      }

    } catch (error) {
      toast.error('Failed to update refund policy. Please try again!');
    }
  }
    ...
    <div className='bg-gray-50 min-h-screen relative px-6'>
      ...
      <div className='max-w-screen-2xl mx-auto py-3 md:py-6 relative'>
        <div className='flex items-center justify-between'>
          <h3 className='w-full font-semibold text-xl lg:text-2xl'>Edit Refund Policy</h3>
          <Link className='flex items-center gap-2 text-[10px] md:text-base justify-end w-full' href={"/dash-board/settings/refund-policy"}> <span className='border border-black hover:scale-105 duration-300 rounded-full p-1 md:p-2'><FaArrowLeft /></span> Go Back</Link>
        </div>
      </div>
      ...
      <form className='2xl:max-w-screen-2xl 2xl:mx-auto relative flex flex-col gap-4' onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className='flex flex-col gap-4 bg-gray-50 rounded-lg h-fit'>
          <label htmlFor='refundPolicy' className='flex justify-start font-medium text-[#9F5216]'>
            Details About Refund Policy *
          </label>
          <Controller
            name="refundPolicy"
            defaultValue=""
            ...
            />}
          />
          {errors.refundPolicy?.type === "required" && (
            <p className="text-red-600 text-left pt-1">Refund Policy is required</ p>
          )}
        </div>
