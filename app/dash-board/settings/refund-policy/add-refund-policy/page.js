...
import useRefundPolicy from '@/app/hooks/useRefundPolicy';
...
const AddRefundPolicy = () => {
  ...
  const [refundPolicyList, isRefundPolicyPending] = useRefundPolicy();

  if (isRefundPolicyPending) return <Loading />;
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (refundPolicyList?.length > 0) {
        toast.error("You cannot add privacy policy, you can edit previous one.")
        return;
      }

      const res = await axiosPublic.post(`/add-refund-policy`, refundPolicyData);
              ...
              <div className="flex items-start">
                <div className="ml-3 flex-1">
                  ...
                  <p className="mt-1 text-sm text-gray-500">
                    Refund Policy has been added successfully!
                  </p>
                </div>
              </div>
            </div>
    ...
    <div className='bg-gray-50 min-h-screen relative px-6'>
      ...
      <div className='max-w-screen-md mx-auto py-3 md:py-6 sticky top-0 z-10 bg-gray-50'>
        <div className='flex items-center justify-between'>
          <h3 className='flex-1 font-semibold text-base md:text-xl lg:text-3xl text-neutral-700'>REFUND POLICY CONFIGURATION</h3>

          <Link // Trigger the modal on click
            className="flex items-center gap-2 text-[10px] md:text-base justify-end"
            href="/dash-board/settings/refund-policy">
