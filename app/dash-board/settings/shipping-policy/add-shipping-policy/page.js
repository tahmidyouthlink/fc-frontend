...
const AddShippingPolicy = () => {
  ...
  const [shippingPolicyList, isShippingPolicyPending] = useShippingPolicy();

  if (isShippingPolicyPending) return <Loading />;
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (shippingPolicyList?.length > 0) {
        toast.error("You cannot add shipping policy, you can edit previous one.")
        return;
      }

      const res = await axiosPublic.post(`/add-shipping-policy`, shippingPolicyData);
              ...
              <div className="flex items-start">
                <div className="ml-3 flex-1">
                  ...
                  <p className="mt-1 text-sm text-gray-500">
                    Shipping Policy has been added successfully!
                  </p>
                </div>
              </div>
            </div>
    ...
    <div className='bg-gray-50 min-h-screen relative px-6'>
      ...
      <div className='max-w-screen-2xl mx-auto py-3 md:py-6 sticky top-0 z-10 bg-gray-50'>
        <div className='flex items-center justify-between'>
          <h3 className='flex-1 font-semibold text-base md:text-xl lg:text-3xl text-neutral-700'>SHIPPING POLICY CONFIGURATION</h3>

          <Link // Trigger the modal on click
            className="flex items-center gap-2 text-[10px] md:text-base justify-end"
            href="/dash-board/settings/shipping-policy">
