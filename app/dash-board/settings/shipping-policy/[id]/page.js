...
const EditShippingPolicyPage = () => {
  ...
  useEffect(() => {
    const fetchShippingPolicyDetails = async () => {
      try {
        const { data } = await axiosPublic.get(`/get-single-shipping-policy/${id}`);

        setValue('pageTitle', data?.pageTitle);
        setValue('shippingPolicy', data?.shippingAndPolicy);

      } catch (error) {
        toast.error("Failed to load shipping policy details.");
      }
    };

    fetchShippingPolicyDetails();
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const shippingPolicyData = {
        ...
        shippingAndPolicy: data?.shippingPolicy,
      }

      const res = await axiosPublic.put(`/update-shipping-policies/${id}`, shippingPolicyData);
      ...
      if (res.data.modifiedCount > 0) {
              ...
              <div className="flex items-start">
                <div className="ml-3 flex-1">
                  ...
                  <p className="mt-1 text-sm text-gray-500">
                    Shipping Policy has been updated successfully!
                  </p>
                </div>
              </div>
            </div>
        ...
        router.push("/dash-board/settings/shipping-policy");
      }
    ...
    <div className='bg-gray-50 min-h-screen relative px-6'>
      ...
      <div className='max-w-screen-md mx-auto py-3 md:py-6 relative'>
        <div className='flex items-center justify-between'>
          <h3 className='w-full font-semibold text-xl lg:text-2xl'>Edit Shipping Policy</h3>
          <Link className='flex items-center gap-2 text-[10px] md:text-base justify-end w-full' href={"/dash-board/settings/shipping-policy"}> <span className='border border-black hover:scale-105 duration-300 rounded-full p-1 md:p-2'><FaArrowLeft /></span> Go Back</Link>
        </div>
      </div>
      ...
      <form className='2xl:max-w-screen-md 2xl:mx-auto relative flex flex-col gap-4' onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className='flex flex-col gap-4 bg-gray-50 rounded-lg h-fit'>
          <label htmlFor='shippingPolicy' className='flex justify-start font-medium text-[#9F5216]'>
            Details About Shipping Policy *
          </label>
          <Controller
            name="shippingPolicy"
            defaultValue=""
            ...
            />}
          />
          {errors.shippingPolicy?.type === "required" && (
            <p className="text-red-600 text-left pt-1">Shipping Policy is required</ p>
          )}
        </div>
...
export default EditShippingPolicyPage;