    ...
    <div className='bg-gray-50 min-h-screen'>
      ...
      <div className='px-6 max-w-screen-2xl mx-auto pb-6'>
        ...
        <div className='sticky top-0 z-10 bg-gray-50 flex items-center justify-between py-6'>
          ...
          {faqList?.length > 0 ? (
            faqList.map((item) => (
              <button key={item._id} onClick={() => router.push(`/dash-board/settings/faq/${item._id}`)}>
            ...
            <button onClick={() => router.push('/dash-board/settings/faq/add-faq')} className="relative z-[1] flex items-center gap-x-3 rounded-lg bg-[#ffddc2] px-[15px] py-2.5 transition-[background-color] duration-300 ease-in-out hover:bg-[#fbcfb0] font-bold text-[14px] text-neutral-700">
