...
const EditReturnPolicyPage = () => {
  ...
  useEffect(() => {
    const fetchReturnPolicyDetails = async () => {
      try {
        const { data } = await axiosPublic.get(`/get-single-return-policy/${id}`);

        setValue('pageTitle', data?.pageTitle);
        setValue('returnPolicy', data?.returnAndPolicy);

      } catch (error) {
        toast.error("Failed to load return policy details.");
      }
    };

    fetchReturnPolicyDetails();
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      const res = await axiosPublic.put(`/update-return-policies/${id}`, returnPolicyData);
              ...
              <div className="flex items-start">
                <div className="ml-3 flex-1">
                  ...
                  <p className="mt-1 text-sm text-gray-500">
                    Return Policy has been updated successfully!
                  </p>
                </div>
              </div>
            </div>
...
export default EditReturnPolicyPage;