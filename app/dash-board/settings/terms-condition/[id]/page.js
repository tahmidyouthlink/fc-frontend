...
const EditTermsConditionsPage = () => {
  ...
  useEffect(() => {
    const fetchTermsNConditionsDetails = async () => {
      try {
        const { data } = await axiosPublic.get(`/get-single-terms-conditions/${id}`);

        setValue('pageTitle', data?.pageTitle);
        setValue('termsCondition', data?.termsAndConditions);

      } catch (error) {
        toast.error("Failed to load this terms and condition details.");
      }
    };

    fetchTermsNConditionsDetails();
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      const res = await axiosPublic.put(`/update-terms-conditions/${id}`, termsData);
    ...
    <div className='bg-gray-50 min-h-screen relative px-6'>
      ...
      <div className='max-w-screen-md mx-auto py-3 md:py-6 relative'>
        <div className='flex items-center justify-between'>
          <h3 className='w-full font-semibold text-xl lg:text-2xl'>Edit Terms & Conditions</h3>
          <Link className='flex items-center gap-2 text-[10px] md:text-base justify-end w-full' href={"/dash-board/settings/terms-condition"}> <span className='border border-black hover:scale-105 duration-300 rounded-full p-1 md:p-2'><FaArrowLeft /></span> Go Back</Link>
        </div>
      </div>
