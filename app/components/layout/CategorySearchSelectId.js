...
const CategorySearchSelectId = ({ categoryList, value = [], onSelectionChange }) => {
  const [searchTerm, setSearchTerm] = useState('');
  const [selectedCategories, setSelectedCategories] = useState(value); // Initialize with the passed value
  ...
  const toggleCategorySelection = (categoryLabel) => {
    const updatedSelectedCategories = selectedCategories.includes(categoryLabel)
        ...
        <div className="border p-2 rounded mt-2">
          ...
          <ul className="space-y-2">
            {selectedCategories.map((label) => (
              <li key={label} className="flex justify-between items-center bg-gray-100 p-2 rounded">
                <span>{label}</span>
                <button
...
export default CategorySearchSelectId;