...
export default function CompleteOutfitProducts({
  ...
  return (
    <section className="relative mt-8 bg-[#fffaf4] py-8 md:mt-12">
      <div
        className={`absolute inset-0 flex h-full w-full items-center justify-between space-y-5 px-5 sm:px-8 lg:px-12 xl:mx-auto xl:max-w-[1200px] xl:px-0 ${completeOutfitProducts?.length < 5 ? "hidden" : "flex"}`}
      >
        <Button
      ...
      <div className="space-y-5 px-5 sm:px-8 lg:px-12 xl:mx-auto xl:max-w-[1200px] xl:px-0">
        ...
        <div
          className={`grid grid-cols-2 gap-x-4 gap-y-12 sm:grid-cols-3 lg:[&>div]:min-w-[calc(25%-16px*3/4)] ${completeOutfitProducts?.length > 3 ? "sm:max-lg:[&>div:last-child]:hidden" : ""} ${completeOutfitProducts?.length < 5 ? "lg:grid-cols-4" : "transition-transform duration-300 ease-in-out lg:flex lg:flex-nowrap"} ${isCompleteOutfitProductsSlid ? "-translate-x-[calc(100%+16px)]" : "-translate-x-0"}`}
        >
          {completeOutfitProducts.map((completeOutfitProduct) => (
            <ProductCard
